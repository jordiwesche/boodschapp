...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedItems = itemsWithProducts.map((item) => ({
      ...
      emoji: !item.product_id ? '🛒' : item.category?.name === 'Overig' ? '📦' : (item.product ? item.product.emoji : (item.category?.name ? CATEGORY_EMOJI_MAP[item.category.name] ?? '🛒' : '🛒')),
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Transform response — if product join didn't work, fetch product separately
    let product = null
    ...
    const transformedItem = {
      ...
      emoji: !item.product_id ? '🛒' : categoryData?.name === 'Overig' ? '📦' : (product ? product.emoji : (categoryData?.name ? CATEGORY_EMOJI_MAP[categoryData.name] ?? '🛒' : '🛒')),
      quantity: item.quantity,
      description: item.description,
      category_id: item.category_id,
      category: categoryData
